...
- **Guest tools are the mirror image of the KVM block, and there is no one package for them.** The KVM/QEMU equivalent of VMware Tools ships as two: `qemu-guest-agent`, the virtio-serial control channel the host uses for graceful shutdown, consistent snapshots and reporting the guest's IP, and `spice-vdagent`, the desktop half — shared clipboard, automatic display resize, drag and drop — which needs the VM to be using a SPICE display before it does anything. `install_vm_guest_tools` is on by default, but the tasks additionally require `ansible_virtualization_role == 'guest'` (systemd-detect-virt, via fact gathering), so on physical hardware the block prints why it skipped and installs nothing. **Neither service is enabled, and that is deliberate — both are udev-activated.** `qemu-guest-agent.service` ships an empty `[Install]` section, so it is `static`: `systemctl enable` prints "not meant to be enabled" and exits 0, and Ansible reads `static` as already-enabled, so `enabled: true` would be a silent no-op reported as ok. `60-qemu-guest-agent.rules` starts it from the virtio-ports device instead, every boot and on hotplug; `spice-vdagentd` is socket-activated the same way from `70-spice-vdagentd.rules`, and its per-session half is a systemd *user* unit wanted by `graphical-session.target`. What udev does not cover is the boot already in progress — its rules fire on device events, and the event for a channel attached before the run has passed — so the playbook starts each unit by hand, guarded on the channel it binds to existing. That guard is load-bearing, not tidiness: `qemu-guest-agent.service` has `BindsTo=` the guest-agent device, so starting it on a VM whose host attached no agent channel is a failed job and a failed play.
...
- **Hermes is the heaviest of the user-scoped installs, and the only one that needs a flag.** `install.sh` doesn't drop a binary the way `claude`, `opencode` and `herdr` do: it clones the repo to `~/.hermes/hermes-agent`, builds a uv venv, fetches its own Node, and writes a launcher shim to `~/.local/bin/hermes` — which is what `creates:` guards. `--skip-setup` is **required**, not tidiness: without it the script ends by running `hermes setup`, an interactive provider login it reads from `/dev/tty` *specifically so that piping the script doesn't skip it*, so `curl | bash` is not protection. Configure it afterwards with `hermes setup --portal`. The prerequisite task (`git`, `xz-utils`, `ripgrep`, `ffmpeg`, `libatomic1`) exists to keep the installer out of apt — it shells out to `sudo apt-get` for whatever is missing and tries to prompt when sudo wants a password; pre-installing means each probe finds its package and returns. It also makes `install_hermes` work with `install_git: false`. Upstream lists Linux x86_64 and aarch64 as tier 1 from the same URL, so nothing arch-specific is needed. **One side effect worth knowing, and it collides with `install_nodejs`:** the installer also symlinks its vendored `node`, `npm` and `npx` into `~/.local/bin`, which precedes `/usr/bin` on the stock `~/.profile` PATH. With both toggles on, your shell resolves `node` to Hermes's bundled copy rather than the NodeSource one — apt still owns `/usr/bin/node` and root's PATH is unaffected, so nothing breaks, but the version won't be the one `nodejs_major` names. The run's summary says so when both are selected. Deleting the three symlinks is enough to prefer NodeSource; Hermes calls its own by absolute path and doesn't need them.
- **Node.js comes from NodeSource, pinned to a major.** The archive's `nodejs` is frozen at release (18.x on noble, past end-of-life) and split from `npm`, which drags a second `nodejs` dependency behind it; NodeSource ships one package carrying both, for amd64 and arm64. `nodejs_major` picks the line — even numbers are LTS, odd ones stop getting fixes within months — and apt upgrades within it from then on. The suite is the literal string `nodistro`, not a codename: NodeSource publishes a single distro-independent index. The playbook also writes `/etc/apt/preferences.d/nodesource` (priority 600, the same file NodeSource's own setup script writes), because otherwise which build wins is left to version-string comparison between two unrelated numbering schemes — that favours NodeSource today, but a release carrying the same major would silently decide it the other way. Set `nodejs_pin_nodesource: false` to leave it to apt. Any legacy `nodesource.list` from an older setup script is deleted first: left beside the `.sources` it names a different keyring for the same URI, which is the Signed-By failure that breaks *every* apt source.
- **Rust is rustup, per user, and needs a linker.** The archive's `rustc` is one frozen compiler with no channel switching, target installs or component management, against an upstream that releases every six weeks — so this is `sh.rustup.rs` run as your user, guarded with `creates:`, self-updating afterwards via `rustup update`. `rust_profile` (`default` or `minimal`) picks what the first install pulls in. `rust_toolchain` keeps meaning something after that first run: the playbook reads `rustup show active-toolchain` and runs `rustup default` when it doesn't match, so changing the variable actually moves the machine. Both are called by absolute path — `~/.cargo/bin` reaches PATH only through `~/.cargo/env`, which a login shell sources and Ansible's non-interactive one doesn't; for the same reason `cargo` is only on *your* PATH after a fresh login. `rust_install_linker` installs `build-essential`, and it matters: `rustc` doesn't link binaries itself, and without `cc` every `cargo build` fails at the link step.
- **Go is upstream's tarball, not `golang-go`.** The archive trails by a release or more and never moves within one (noble is on 1.22), and Go's compatibility promise makes tracking current cheap. `go_version` defaults to `latest`, resolved from `https://go.dev/VERSION?m=text` at run time; pin a bare number (`-e go_version=1.25.1`) to hold a machine still. The playbook compares against `go version` — whitespace-split tokens, not a substring, since `go1.25` occurs inside `go1.25.1` — removes `/usr/local/go` and unpacks the new tarball only on a mismatch. Removal first is upstream's documented upgrade step: the tarball carries no manifest of what it replaces, so an in-place extract would leave files from the old release on PATH. It's `curl | tar` in a `shell` task rather than `ansible.builtin.unarchive` on purpose — `unarchive` fetches the archive *before* it consults check mode, so `--check --diff` would download 80 MB to decide it was going to change something, where a `shell` task is simply skipped. PATH is system-wide in `/etc/profile.d/go.sh` (`go_profile_script`), because `/usr/local/go` is root-owned and shared; the same file adds `$HOME/go/bin`, which expands in each reader's own shell. Login shells only, so `go` appears after a fresh session.
- **`creates:` paths are absolute, resolved from `getent passwd`.** The `command`/`shell` modules test `creates:` with `glob.glob()`, which does **not** expand `~` — a tilde path there matches nothing, so the guard never fires and the installer it guards re-runs on every play, reporting `changed` forever. `pre_tasks` resolves `target_user_home` once and the five user-scoped installs (Claude Code, OpenCode, Hermes, Herdr, rustup) use it literally. Like `target_user` and `ansible_become_exe`, that lookup reads the machine Ansible is connected to, which is the target only because the inventory is `localhost ansible_connection=local`.
